...
// was not loaded in eager-loading, or loaded but was not found.
func (e UserRoleEdges) GrantedByUserOrErr() (*User, error) {
...
// was not loaded in eager-loading, or loaded but was not found.
func (e UserRoleEdges) GrantedByServicePrincipalOrErr() (*ServicePrincipal, error) {
...
func (*UserRole) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case userrole.FieldID, userrole.FieldUserID, userrole.FieldRoleID, userrole.FieldGrantedByUserID, userrole.FieldGrantedByServicePrincipalID:
			values[i] = new(sql.NullInt64)
		case userrole.FieldCreatedAt, userrole.FieldUpdatedAt, userrole.FieldExpiresAt:
...
func (_m *UserRole) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case userrole.FieldGrantedByUserID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
		...
		case userrole.FieldGrantedByServicePrincipalID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
...
// QueryGrantedByUser queries the "granted_by_user" edge of the UserRole entity.
func (_m *UserRole) QueryGrantedByUser() *UserQuery {
	return NewUserRoleClient(_m.config).QueryGrantedByUser(_m)
}

// QueryGrantedByServicePrincipal queries the "granted_by_service_principal" edge of the UserRole entity.
func (_m *UserRole) QueryGrantedByServicePrincipal() *ServicePrincipalQuery {
	return NewUserRoleClient(_m.config).QueryGrantedByServicePrincipal(_m)
}
