...
func newOptionalJWTTestEnv(t testing.TB, users map[int64]*service.User) (*gin.Engine, *service.AuthService) {
	t.Helper()
	...
	actorResolver, _ := newMiddlewareActorResolver(t, users)
	mw := NewOptionalJWTAuthMiddleware(authSvc, userSvc, nil, nil, actorResolver)
...
func TestOptionalJWTAuth_NoHeaderPassesAnonymously(t *testing.T) {
	router, _ := newOptionalJWTTestEnv(t, nil)
	...
	require.Contains(t, w.Body.String(), `"has_actor":false`)
}
...
func TestOptionalJWTAuth_ValidTokenSetsSubject(t *testing.T) {
	...
	router, authSvc := newOptionalJWTTestEnv(t, map[int64]*service.User{7: user})
	...
	require.Contains(t, w.Body.String(), `"actor_user_id":7`)
}
