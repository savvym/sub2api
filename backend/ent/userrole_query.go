...
func (_q *UserRoleQuery) QueryGrantedByUser() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserRoleQuery) QueryGrantedByServicePrincipal() *ServicePrincipalQuery {
...
func (_q *UserRoleQuery) WithGrantedByUser(opts ...func(*UserQuery)) *UserRoleQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserRoleQuery) WithGrantedByServicePrincipal(opts ...func(*ServicePrincipalQuery)) *UserRoleQuery {
	query := (&ServicePrincipalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserRoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserRole, error) {
	...
	if query := _q.withGrantedByUser; query != nil {
		if err := _q.loadGrantedByUser(ctx, query, nodes, nil,
			func(n *UserRole, e *User) { n.Edges.GrantedByUser = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withGrantedByServicePrincipal; query != nil {
		if err := _q.loadGrantedByServicePrincipal(ctx, query, nodes, nil,
			func(n *UserRole, e *ServicePrincipal) { n.Edges.GrantedByServicePrincipal = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserRoleQuery) loadGrantedByUser(ctx context.Context, query *UserQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UserRole)
	for i := range nodes {
...
func (_q *UserRoleQuery) loadGrantedByServicePrincipal(ctx context.Context, query *ServicePrincipalQuery, nodes []*UserRole, init func(*UserRole), assign func(*UserRole, *ServicePrincipal)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*UserRole)
	for i := range nodes {
...
func (_q *UserRoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withGrantedByUser != nil {
			_spec.Node.AddColumnOnce(userrole.FieldGrantedByUserID)
		}
		if _q.withGrantedByServicePrincipal != nil {
			_spec.Node.AddColumnOnce(userrole.FieldGrantedByServicePrincipalID)
		}
	}
