...
func (h *AccountHandler) SyncUpstreamModels(c *gin.Context) {
	...
	catalog, err := h.accountTestService.AdminSyncUpstreamModelCatalog(c.Request.Context(), actor, account)
	if err != nil {
...
func (h *AccountHandler) SyncUpstreamModelsPreview(c *gin.Context) {
	...
	catalog, err := h.accountTestService.AdminSyncUpstreamModelCatalog(c.Request.Context(), actor, tempAccount)
	if err != nil {
		var syncErr *service.UpstreamModelSyncError
		if errors.As(err, &syncErr) {
			switch syncErr.Kind {
