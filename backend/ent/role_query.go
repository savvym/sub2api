...
type RoleQuery struct {
	...
	withUsers                 *UserQuery
	withServicePrincipals     *ServicePrincipalQuery
...
// QueryServicePrincipals chains the current query on the "service_principals" edge.
func (_q *RoleQuery) QueryServicePrincipals() *ServicePrincipalQuery {
...
func (_q *RoleQuery) QueryServicePrincipalRoles() *ServicePrincipalRoleQuery {
...
func (_q *RoleQuery) Clone() *RoleQuery {
	...
	return &RoleQuery{
		...
		withServicePrincipals:     _q.withServicePrincipals.Clone(),
...
// WithServicePrincipals tells the query-builder to eager-load the nodes that are connected to
// the "service_principals" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *RoleQuery) WithServicePrincipals(opts ...func(*ServicePrincipalQuery)) *RoleQuery {
	query := (&ServicePrincipalClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withServicePrincipals = query
	return _q
}
...
func (_q *RoleQuery) WithServicePrincipalRoles(opts ...func(*ServicePrincipalRoleQuery)) *RoleQuery {
	query := (&ServicePrincipalRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withServicePrincipals; query != nil {
		if err := _q.loadServicePrincipals(ctx, query, nodes,
			func(n *Role) { n.Edges.ServicePrincipals = []*ServicePrincipal{} },
			func(n *Role, e *ServicePrincipal) { n.Edges.ServicePrincipals = append(n.Edges.ServicePrincipals, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withServicePrincipalRoles; query != nil {
		if err := _q.loadServicePrincipalRoles(ctx, query, nodes,
			func(n *Role) { n.Edges.ServicePrincipalRoles = []*ServicePrincipalRole{} },
			func(n *Role, e *ServicePrincipalRole) {
...
func (_q *RoleQuery) loadServicePrincipals(ctx context.Context, query *ServicePrincipalQuery, nodes []*Role, init func(*Role), assign func(*Role, *ServicePrincipal)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Role)
	nids := make(map[int64]map[*Role]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(role.ServicePrincipalsTable)
		s.Join(joinT).On(s.C(serviceprincipal.FieldID), joinT.C(role.ServicePrincipalsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(role.ServicePrincipalsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(role.ServicePrincipalsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "service_principals" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *RoleQuery) loadServicePrincipalRoles(ctx context.Context, query *ServicePrincipalRoleQuery, nodes []*Role, init func(*Role), assign func(*Role, *ServicePrincipalRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(serviceprincipalrole.FieldRoleID)
	}
	query.Where(predicate.ServicePrincipalRole(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.ServicePrincipalRolesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
