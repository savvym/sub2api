...
- [x] 2.1 实现 hoster 资格、帐号/分组配额和管理员分配入口。
- [x] 2.2 实现普通用户私有帐号 CRUD、字段投影和简化创建向导。
- [x] 2.3 实现普通用户私有分组 CRUD，并限制平台策略字段。
- [x] 2.4 覆盖 OAuth、导入、复制、批量和 callback 的可信 Owner 绑定。
- [x] 2.5 按需创建 Owner 私有默认分组，帐号创建与绑定同事务。
