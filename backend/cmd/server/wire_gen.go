...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	authorizationPropagationStatsRepository := repository.NewAuthorizationPropagationStatsRepository(db)
	...
	authorizationExpiryRepository := repository.NewAuthorizationExpiryRepository(db)
	authorizationExpiryWorker := service.ProvideAuthorizationExpiryWorker(authorizationExpiryRepository)
	authorizationPropagationGuard := service.ProvideAuthorizationPropagationGuard(authorizationPropagationStatsRepository, authCacheInvalidationWorker, schedulerSnapshotService, authorizationExpiryWorker)
	settingService := service.ProvideSettingService(settingRepository, groupRepository, proxyRepository, configConfig, authorizationPropagationGuard)
	...
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, authCacheInvalidationWorker, apiKeyService, authorizationPropagationGuard)
	...
	roleRepository := repository.NewRoleRepository(client, db)
	...
	resourceMutationRepository := repository.NewResourceMutationRepository(client)
	actorResolverStore := repository.NewAuthzActorResolverStore(client)
	resolver := authz.NewActorResolver(actorResolverStore)
	policyStore := repository.NewAuthzPolicyStore(client, configConfig)
	roleShadowObserver := service.NewAuthorizationRoleShadowObserver()
	resourcePolicy := service.ProvideResourcePolicy(policyStore, roleShadowObserver)
	resourceMutationCoordinator := service.ProvideResourceMutationCoordinator(resourceMutationRepository, resolver, resourcePolicy, authorizationPropagationGuard)
	adminService := service.NewAdminService(userRepository, roleService, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, compositeModelRouteRepository, compositeRouteResolver, channelService, resourceMutationCoordinator)
	...
	authorizationHandler := admin.NewAuthorizationHandler(roleService, totpService, userService)
	hostingEntitlementRepository := repository.NewHostingEntitlementRepository(client)
	hostingEntitlementService := service.NewHostingEntitlementService(hostingEntitlementRepository, resolver, resourcePolicy)
	hostingEntitlementHandler := admin.NewHostingEntitlementHandler(hostingEntitlementService, totpService, userService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, authorizationHandler, hostingEntitlementHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	selfServiceAccountRepository := repository.NewSelfServiceAccountRepository(client)
	scopedResourceReader := repository.NewScopedResourceReader(client)
	scopedAccountReader := repository.ProvideScopedAccountReader(scopedResourceReader)
	scopedGroupReader := repository.ProvideScopedGroupReader(scopedResourceReader)
	resourceReadService := service.NewResourceReadService(resourcePolicy, scopedAccountReader, scopedGroupReader)
	...
	openAIQuotaAutoResetFinalizer := repository.NewOpenAIQuotaAutoResetFinalizer(db)
	openAIQuotaAutoResetAccountLocker := repository.NewOpenAIQuotaAutoResetAccountLocker(db)
	workerPolicyStore := repository.NewAuthzWorkerPolicyStore(client)
	workerPolicy := service.ProvideWorkerPolicy(workerPolicyStore)
	openAIQuotaAutoResetService, err := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, openAIQuotaAutoResetFinalizer, openAIQuotaAutoResetAccountLocker, auditLogService, settingService, leaderLockCache, resolver, workerPolicy)
	if err != nil {
		return nil, err
	}
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, authorizationExpiryWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, openAIQuotaAutoResetService, promptService, pluginManager)
