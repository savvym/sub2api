...
| OpenAI quota auto-reset | 固定、roleless、可停用的 `openai_quota_auto_reset_worker` Service Principal；只拥有专用 `platform.account.openai_quota_auto_reset` direct worker permission，额外角色/权限均 fail closed | scanner/实时 signal 的 query/reset/recover/cache/load/update 和幂等新操作逐次重授权。普通流程三次 mutable eligibility reload，第四次在 POST 前的 proxy `FOR SHARE` + account `FOR UPDATE` 锁内复核 eligibility 与完整 upstream identity；evaluate 前半程只持 advisory lease，POST 返回立即释放整笔 advisory+row transaction，agent retry 取得新 lease。独立 recovery keyset pager 不依赖 enabled/status/schedulable，使用可取消阻塞式可靠入队；managed-state parser strict 拒绝未知字段/类型/状态、坏 hash pair 和超界 count。Actor-qualified scope 与稳定 redeem request ID 防重复；唯一例外是已发生 reset 的原子事实落账，finalizer 不重新授权但不得执行恢复等新副作用 | 当前工作树运行时工程接入、focused/5 个 PG finalizer 回归、migration 默认测试及完整 integration-tag migrations（本机 PostgreSQL 18.6，23.704s）通过。migration 243 marker 固化帐号 provenance；success/no-effect 各自强制专用 deterministic decision ID，不复用 redeem request ID。success 终态由数据库固定保留 8 天，且在同 stable-key 的 `resetting|failed` recovery state 存在时继续保留。no-effect 的 drained 参数只是调用方断言；执行前必须归档 fleet 排空和 upstream 无效果证据，并在 audit extra 留不含凭据的 evidence ref/decision owner。discard 仅在 stable-key 精确匹配时清当前 managed state，unknown 保持保护。生产迁移前五列 provenance inventory 必须全部 `resolved`，三列 terminal-recovery inventory 任一行都阻断；处理路由见 `implementation-evidence.md`。迁移后 unresolved=0、当前 SHA CI/Testcontainers/Security Scan 与非混合版本发布证据仍待归档，不据此勾选 1.12 |

上述协调权限还依赖数据库强制的 audit 身份：account/record/fingerprint 必须精确绑定，`evidence_ref` 和 `decision_owner` 必须是非空限长 string，否则整事务回滚。confirmed-success 只保留八参数 `reconcile_openai_quota_auto_reset_protected_attempt(bigint,text,text,bigint,timestamptz,text,integer,jsonb)`；raw reapply 显式删除两个废弃 overload，不能借旧入口绕过数据库构造的终态或固定 8 天 retention。

1.11 没有宣称上述其他后台路径已全部迁移。核心 HTTP 管理命令的事务协调和三个传播 Worker 不能替代 token refresh、quota/rate-limit、probe、CRS、monitor 等路径逐项定义受限 Actor、Policy、版本和 Outbox 责任。main 合并时识别的 OpenAI quota auto-reset Actor 缺口已在当前工作树按上表收口，但这不代表其余后台写路径完成，也不替代当前 SHA 的发布验证和正式 Phase 1 批准。
